...
# from tortoise.enums import CharEnum

# class Gender(CharEnum):
#     MAN = '1'
#     WOMAN = '0'
#     SECRECY = '2'
...
class Users(Model):
    ...
    nickname = fields.CharField(default=None, null=True,max_length=10, description='用户昵称')
    avatar = fields.BinaryField(default=None, null=True, description='用户头像')
    birthday = fields.DateField(default=None, null=True, description='生日')
    introduction = fields.TextField(default=None, null=True, description='个人介绍')
...
class Articles(Model):
    ...
    ucomment_count = fields.IntField(default=0, description='评论数')


class ArticleImages(Model):
    image_id = fields.IntField(primary_key=True, auto_increment=True, description='图片ID')
    article = fields.ForeignKeyField('models.Articles', related_name='articleImages', on_delete=fields.CASCADE, description='关联文章ID')
    image_data = fields.BinaryField(description='图片二进制数据')
    upload_time = fields.DatetimeField(auto_now_add=True, description='上传时间')
    description = fields.CharField(max_length=255, null=True, description='图片描述')
...
class Tags(Model):
    ...
    article = fields.ManyToManyField('models.Articles', related_name='tags', through='article_tag', on_delete=fields.CASCADE, description='文章和标签外键')
...
class Collects(Model):
    ...
    article = fields.ForeignKeyField('models.Articles', related_name='collects', on_delete=fields.SET_NULL, null=True, description='收藏文章id')


class Comments(Model):
    comment_id = fields.IntField(primary_key=True, auto_increment=True, description='评论id')
    user = fields.ForeignKeyField('models.Users', related_name='comments', on_delete=fields.SET_NULL, null=True, description='发表评论用户id')
